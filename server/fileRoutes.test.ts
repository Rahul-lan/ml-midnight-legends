...
const storagePut = vi.fn(async () => ({ key: "user-7/files/key", url: "/manus-storage/user-7/files/key" }));
const storageGetSignedUrl = vi.fn(async (key: string) => `https://signed.example/${key}`);
vi.mock("./_core/sdk", () => ({ sdk: { authenticateRequest: vi.fn(async () => ({ id: 7 })) } }));
...
describe("public file route integration", () => {
  it("carries an authenticated upload into the anonymous public flow", async () => {
    ...
    expect(insertFile).toHaveBeenCalledWith(expect.objectContaining({ userId: 7, name: "hello.txt", size: 5, mimeType: "text/plain", storageKey: "user-7/files/key" }));
    ...
    await expect(visitor.files.publicDownload({ id: 42 })).resolves.toEqual({ url: "https://signed.example/user-7/files/key" });
    expect(publicResponse.redirect).toHaveBeenCalledWith(302, "https://signed.example/user-7/files/key");
  });
});
